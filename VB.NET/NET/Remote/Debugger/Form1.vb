...
#End Region
    ...
    Private Sub Form1_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        For i = 0 To files.Length - 1
            files(i) = GetSetting("idectl", "Remote", "FileMRU" & i + 1)
        Next i
        ...
        BeginInvoke(New MethodInvoker(AddressOf Configure))
    End Sub
    ...
    Private Sub BasicIdeCtl1_Disconnecting(ByVal sender As Object, ByVal e As System.EventArgs) Handles BasicIdeCtl1.Disconnecting
        ...
        Dim i As Integer
        For i = 0 To files.Length - 1
            SaveSetting("idectl", "Remote", "FileMRU" & i + 1, files(i))
        Next i
        ' ***
    End Sub

    Private Sub Configure()
        Dim Config As ConfigForm = New ConfigForm(Me)
    ...
    Public Function Initialize(ByVal ipAddr As String, ByVal port As Integer) As Boolean
        Dim Skt As Socket = New Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp)
        Try
            Dim ip As IPAddress = IPAddress.Parse(ipAddr)
            Dim ep As IPEndPoint = New IPEndPoint(ip, port)
            Skt.Connect(ep)
            m_conn = New Connection(Skt, New ReceivedDataDelegate(AddressOf ReceivedDataAsync))
            Return True
    ...
    Public Sub ReceivedData(ByVal conn As Connection, ByVal data As String)
        BasicIdeCtl1.Synchronize(data, 0) ' id is ignored
    End Sub
...
#End Region
    ...
    Private Sub basicIdeCtl1_Synchronizing(ByVal sender As Object, ByVal e As WinWrap.Basic.Classic.SynchronizingEventArgs) Handles BasicIdeCtl1.Synchronizing
        m_conn.Send(e.Param)
    End Sub
End Class
